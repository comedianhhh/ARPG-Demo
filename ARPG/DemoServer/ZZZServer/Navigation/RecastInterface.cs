...
public class RecastInterface
{
    ...
    public static Vector3d SPos = new Vector3d();
    public static Vector3d EPos = new Vector3d();
    ...
    /// <returns></returns>
    public static bool FindPath(int id, ref Vector3d spos, ref Vector3d epos)
    {
        ...
        {
            Console.WriteLine($"错误:Recast寻路 FindPath Error：- 起点非法 - spos:{spos} - MapId:{id}");
        }

        SPos = spos;

        fixPos = fixposition(id, epos);
        if (fixPos != null)
        {
            epos.y = fixPos[1];
        }
        ...
        {
            Console.WriteLine($"错误:Recast寻路 FindPath Error：- 终点非法 - epos:{epos} - MapId:{id}");
        }

        EPos = epos;

        dtStatus status = (dtStatus) recast_findpath(id, new[] { (float)-spos.x, (float)spos.y, (float)spos.z }, new[] { (float)-epos.x, (float)epos.y, (float)epos.z });
        if (dtStatusFailed(status))
        {
            ...
            Console.WriteLine($"错误:{_strLastError}");
            return false;
        }
        if (dtStatusInProgress(status))
        {
            Console.WriteLine($"错误:Recast寻路 Error：寻路尚未结束!- MapId:{id}");
            return false;
        }

        Console.WriteLine($"dtStatus: {(uint)status:B}");
    ...
    /// <param name="epos">终点三维坐标</param>
    /// <returns></returns>
    public static bool Raycast(int id, Vector3d spos, Vector3d epos)
    {
        dtStatus status = (dtStatus) recast_raycast(id, new float[] { (float)-spos.x, (float)spos.y, (float)spos.z }, new float[] { (float)-epos.x, (float)epos.y, (float)epos.z });
        if (dtStatusFailed(status))
        {
            dtStatus statusDetail = status & dtStatus.DT_STATUS_DETAIL_MASK;
    ...
    public static float[] getHitPosition(int id)
    {
        unsafe
        {
            ...
            {
                Console.WriteLine($"错误:RecastInterface getHitPosition Exception! - {e}");
                return null;
            }
        }
    }
    ...
    /// <param name="id">地图Id</param>
    /// <returns></returns>
    public static float[] fixposition(int id, Vector3d pos)
    {
        unsafe
        {
            try
            {
                IntPtr fixPos = recast_getfixposition(id, new float[] { (float)-pos.x, (float)pos.y + 0.5f, (float)pos.z }); // （pos.y+1）抬高一点计算
            ...
            {
                Console.WriteLine($"错误:RecastInterface fixposition Exception! - {e}");
                return null;
            }
        }
    }
    ...
    public static int[] GetPathPoly(int id, out int polyCount)
    {
        unsafe
        {
            ...
            {
                polyCount = 0;
                Console.WriteLine($"错误:RecastInterface GetPathPoly Exception! - {e}");
                return null;
            }
        }
    }
    ...
    public static float[] GetPathSmooth(int id, out int smoothCount)
    {
        unsafe
        {
            ...
            {
                smoothCount = 0;
                Console.WriteLine($"错误:RecastInterface GetPathSmooth Exception! - {e}");
                return null;
            }
        }
    }
